...
class DeepQ:
    ...
    def calculateTargetDouble(self,qValuesNewState, reward, isFinal,qValuesNewStateUpdated):
        if isFinal:
            return reward
        else : 
            return reward + self.discountFactor * qValuesNewState[np.argmax(qValuesNewStateUpdated)]
    ...
    def learnOnMiniBatchDouble(self, miniBatchSize, useTargetNetwork=True):    
        if self.memory.getCurrentSize() > self.learnStart:
            ...
            for sample in miniBatch:
                ...
                qValuesNewStateUpdated = self.getQValues(newState)
                
                if useTargetNetwork:
                    qValuesNewState = self.getTargetQValues(newState)
                else :
                    qValuesNewState = self.getQValues(newState)
                targetValue = self.calculateTargetDouble(qValuesNewState, reward, isFinal,qValuesNewStateUpdated)
...
for epoch in xrange(epochs):
    ...
    for t in xrange(steps):
        ...
        if stepCounter >= learnStart:
            if stepCounter <= updateTargetNetwork:
                deepQ.learnOnMiniBatchDouble(minibatch_size, False)
            else :
                deepQ.learnOnMiniBatchDouble(minibatch_size, True)
    ...
    if(epoch%10 == 0):
        deepQ.model.save('modelsDouble/model'+str(epoch)+'.h5')
